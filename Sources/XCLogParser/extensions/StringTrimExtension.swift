...
extension String {
    ...
    /// # Why this exists
    ///
    /// `parseAsString` is the single largest allocation site in the parser - 20.5% of all
    /// allocations on the flagged benchmark log (438,321 of 2,137,662) - and almost every string
    ...
    /// allocates one object rather than zero. Measured over 100,000 calls on a heap-built String:
    /// 100,007 `swift_allocObject` when there was nothing to trim, 200,007 when there was. So the
    /// fast path removes one of the two allocations. (An earlier estimate assumed a no-op trim was
    /// free and would have predicted twice the real gain.)
    ///
    ...
    /// converting the string to UTF-16 one scalar at a time. On Linux that cost put this one call
    /// over 55% of the parse stage. So when both ends are ASCII the trim is done on the bytes here.
    ///
    ...
    func trimmedIfNeeded() -> String {
        let trimmed: String? = utf8.withContiguousStorageIfAvailable { bytes -> String? in
            guard let first = bytes.first, let last = bytes.last else {
                ...
                // for this was the single most common way into the slow path - 29.6% of all
                // string tokens on the benchmark log, more than three times the number that
                // needed any trimming at all.
                return self
            }
