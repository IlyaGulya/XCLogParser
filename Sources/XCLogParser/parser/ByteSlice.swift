...
///
/// It keeps its own private copy, as a `static` on `Notice`. Do not deduplicate it into this
/// extension: routing its two calls through here adds tens of thousands of allocations, because it
/// runs once per section where the other callers do not.
///
/// The cause is the receiver, not the file or the call syntax - `@inline(__always)`, `@inlinable`
/// and moving the method beside its caller all leave the count unchanged, while making it a
/// `static` on a concrete type in this module recovers it. A method on
/// `UnsafeBufferPointer<UInt8>`, a generic standard-library type, does not optimise the same way
/// for a call made once per section.
///
