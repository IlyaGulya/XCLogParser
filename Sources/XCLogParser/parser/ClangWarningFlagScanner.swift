...
extension Notice {
    ...
    /// dominated it: DTrace time profiling put `parseClangWarningFlags` at **6.52% of all samples** in
    ...
    /// the text. An earlier version did, and it was close to useless. Real logs are overwhelmingly
    /// ASCII but not entirely - 0.026% of bytes in a 278MB fleet log - and those bytes are spread
    /// widely enough that almost every section contained one, so almost every section still went
    /// through ICU: profiling showed the regex still at 22.8% of samples. Elsewhere in the text a
    /// non-ASCII byte cannot affect the result, because no match can begin on one.
    ///
