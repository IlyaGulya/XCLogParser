...
extension IDEActivityLogSection {
    ...
    func getTargetFromCommand() -> String? {
        // Byte-wise search to avoid two Foundation `range(of:)` scans per section;
        // `range(of:)` across its call sites was 11.46% of parse samples. This also
        // skips the second search when the first marker is absent, which the
        // original did not - both `guard` conditions were evaluated eagerly.
