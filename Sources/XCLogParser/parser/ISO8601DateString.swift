...
/// attributed **7.30% of all samples** in a full parse to `NSDateFormatter`, the
...
///   7.30% was steady-state formatting, not setup.
///
...
enum ISO8601DateString {
    ...
    static func format(timeIntervalSinceReferenceDate timeInterval: Double) -> String {
        ...
        // Written into a fixed-size stack buffer rather than an array, because for a
        // format this small the allocations dominate everything else. An earlier
        // version built a `[UInt8]` and called a helper that allocated a scratch
        // array per field - about 8 allocations per date, twice per section. DTrace
        // put that version at 4.21% of samples, of which the arithmetic was 0.10%:
        // the rest was malloc/free, swift_release, _ContiguousArrayStorage deinit
        // and the copy-on-write uniqueness checks behind `Array.append`.
